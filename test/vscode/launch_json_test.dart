...
void main() {
  ...
  group('.vscode/launch.json', () {
    test('pathes in launch.json', () {
      ...
      // Ensure there is a configuration for executing bin/gg_console_colors.dart
      final configurations =
          parsedLaunchJson['configurations'] as List<dynamic>;

      final ggConsoleColors = configurations.firstWhere(
        (dynamic configuration) =>
            configuration['name'].toString() == 'gg_console_colors.dart',
      );

      expect(
        ggConsoleColors,
        isNotNull,
        reason: 'Item for executing bin/gg_console_colors.dart not found',
      );

      expect(ggConsoleColors['name'], 'gg_console_colors.dart');
      expect(ggConsoleColors['type'], 'dart');
      expect(ggConsoleColors['request'], 'launch');
